...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "output" / "ball"
...
def load_model(model_path, device, input_height=288, input_width=512):
...
def preprocess_input(frame_buffer, device, input_height=288, input_width=512):          
...
def main():
    ...
    model = load_model(args.model_path, device, input_height, input_width)
    model.eval()
    ...
    # Sliding-window inference predicts center frames only, then adds first/last fallback frames.
    expected_pred_frames = max(0, total_frames - 2)
    pbar = tqdm(total=expected_pred_frames, desc="Processing video", unit="frame")
    stop = False
    while cap.isOpened() and stop == False:
        ...
        if len(processed_frame_buffer) == 3:
            input_tensor = preprocess_input(processed_frame_buffer, device)
        ...
        # One center frame is produced per loop once the 3-frame window is available.
        pbar.update(1)

    # Ensure the progress bar reaches 100% even if the loop exits early.
    if pbar.n < pbar.total:
        pbar.update(pbar.total - pbar.n)
    pbar.close()

    # Pad tail frames when total frame count is not divisible by 3.
    # The current inference consumes frames in groups of 3, so up to 2 frames can remain.
    while len(frame_buffer) > 0:
        ...
        if args.visualize or out_writer is not None:
            vis_frame = frame.copy()
            vis_frame = draw_track(vis_frame, track_points, current_ball_bbox=None)
            if args.visualize:
    ...
    # Final summary for easier CLI confirmation.
    print(f"Done. Output rows: {frame_index}/{total_frames}")
