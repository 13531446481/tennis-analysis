...
from rtmlib.rtmlib.visualization.skeleton import coco17
...
def detect_kpt(video, balls):
    ...
    NOTE: demo.py 不存在，此函数已被禁用。需要实现或导入正确的 detect_pose 函数。
    """
    # from demo import detect_pose  # ❌ demo.py 文件不存在，需要修复
    ...
    raise NotImplementedError("detect_kpt 函数需要正确的 detect_pose 实现")
...
def read_ball_points(file_name: str) -> np.ndarray:
    """
    读取球轨迹数据：优先 CSV（predict.py 输出），不存在时回退 NPY。

    CSV 支持列: Frame, Visibility, X, Y
    """
    csv_candidates = [
        PROJECT_ROOT / "output" / "ball" / f"{file_name}_predict_ball.csv",
        PROJECT_ROOT / "output" / "tracknetv4_pytorch" / f"{file_name}_predict_ball.csv",
        PROJECT_ROOT / "output" / "tracknetv4" / f"{file_name}_predict_ball.csv",
    ]
        ...
        if csv_path.is_file():
            ...
            with open(csv_path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    x = int(float(row["X"])) if vis == 1 else 0
                    y = int(float(row["Y"])) if vis == 1 else 0
                    points.append([x, y])
            return np.asarray(points, dtype=np.int64)

    npy_path = PROJECT_ROOT / "output" / "ball" / f"{file_name}.npy"
    if npy_path.is_file():
        return np.load(str(npy_path), allow_pickle=True)

    searched = "\n".join([str(p) for p in csv_candidates] + [str(npy_path)])
    raise FileNotFoundError(f"Ball data not found. Searched:\n{searched}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze ball trajectory from video")
    parser.add_argument(
        "--video_path",
        type=str,
        default="videos/002.mp4",
        help="Video path relative to project root or absolute path",
    )
    args = parser.parse_args()
    draw_ball_trajectory(args.video_path)
